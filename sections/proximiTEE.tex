...
Although the above-outlined attestation scheme is useful, it also has a known weakness. %As is shown in Figure~\ref{fig:SystemModel}, 
...
%
The verifier cannot notice such \emph{relay attacks}, since the attestation mechanism is based on a group signature scheme, and all processors from the same group produce indistinguishable signatures. (Even if attestation used traditional digital signatures, it would be very difficult for the verifier to know which signing key corresponds to which CPU.) \red{\#R2 note that in this article, we use Intel SGX to demonstrate relay. However, other TEEs such as ARM TrustZone, RISC-V keystone, etc. do not provide any means for platform identification, hence, vulnerable to relay.}
...
Our first observation is that by re-directing the attestation to the adversary's platform, the adversary enables \emph{physical} side-channel attacks such as acoustic, electric and electromagnetic monitoring \red{\#R1 which are otherwise not possible to mount on the victim's platform. These side channels} have been shown to be effective and inexpensive means to extract secrets from modern PC platforms~\cite{genkin2016physical}. Hardening enclaves against physical side channels is difficult. 
...
\red{\#R1 One strawman solution is to use the SGX local attestation using a challenger enclave. This will prove if the application enclave and the challenger enclave are running on the same platform. But this solution won't work because deploying the challenger enclave requires attestation to that enclave where the attacker can mount the relay attack. Hence, the only way to solve the problem is to introduce a trusted device that is \emph{external} to the platform to prove the proximity.}
Our solution uses a simple embedded device called \key that is attached to the target platform over a local communication interface like USB. \red{\#R1 In hardened remote attestation}, the verifier first establishes a secure channel a \key whose public key the verifier learns from its issuer. \key performs standard remote attestation on the local enclave, establishes a secure TLS channel to it, and verifies its proximity using a simple distance-bounding protocol that consist of repeated and fast challenge-response rounds. If such proximity verification succeeds, \key facilitates the creation of a secure channel between the remote verifier and the attested enclave. 
