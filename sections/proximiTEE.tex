...
\update{4}{One strawman solution is to use the SGX local attestation using a challenger enclave. This will prove if the application enclave and the challenger enclave are running on the same platform. But this solution won't work because deploying the challenger enclave requires attestation to that enclave where the attacker can mount the relay attack. Hence, the only way to solve the problem is to introduce a trusted device that is \emph{external} to the platform to prove the proximity.}
