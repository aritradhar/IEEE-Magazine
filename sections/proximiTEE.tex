...
\update{4}{Proximity verification using an external device overcomes the main problems of TOFU approaches. First, the TCB remains small, as the verifier device can be single-purpose and thus simple. Second, adoption of such solution is easy, as there is no need to fully reset the target platform. And third, the authority that certifies the verifier device can remain fully offline.}
