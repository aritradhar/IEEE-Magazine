...
We implemented a prototype of \protection, which is shown in Figure~\ref{fig:prototype}. The prototype includes a Raspberry Pi board as a computing unit, an Arduino board as a keyboard and mouse event input interceptor, and a separate HDMI interceptor board. Such a prototype implementation shows that the proposed functionalities are feasible to implement on low-cost hardware and that the TCB of the \hub device remains small which is mandatory for the trustworthiness of the solution. Full details of the \protection system are explained in~\cite{protection}.
